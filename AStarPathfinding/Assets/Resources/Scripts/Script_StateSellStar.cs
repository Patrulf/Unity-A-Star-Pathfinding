...
public class Script_StateSellStar : Script_IState {
	...
	public Script_StateSellStar(Script_Grid p_grid, Script_GameManager p_manager, Script_StarChaser p_creator)
	{
	...
	public override void Enter()
	{
		...
		if (_manager.GetEntityOfType<Script_TradingPost> () != null) {
			FindPath (_gridPosition, _manager.GetEntityLocation<Script_TradingPost> ());
	...
	private void DetermineStateChange()
	{
		...
		if (_pathList != null && _pathList.Count > 0) {
			...
			if (Vector3.Distance (_creator.GetPosition(), endOfPathNodePosition) < _creator.GetNodeCollisionRange() ) {		
				...
				if (_manager.GetEntityOfType<Script_TradingPost> () != null) {
					if (_manager.GetEntityLocation<Script_TradingPost> () == _gridPosition) {						
