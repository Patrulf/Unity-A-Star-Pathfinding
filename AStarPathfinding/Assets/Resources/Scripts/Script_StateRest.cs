...
public class Script_StateRest : Script_IState {
	...
	public override void Enter()
	{
		...
		if (_manager.GetEntityOfType<Script_SpaceShip> () != null) {
			FindPath (_gridPosition, _manager.GetEntityLocation<Script_SpaceShip> ());
	...
	private void DetermineStateChange()
	{
		...
		if (_pathList != null && _pathList.Count > 0) {
			...
			if (Vector3.Distance (_creator.GetPosition(), endOfPathNodePosition) < _creator.GetNodeCollisionRange() ) {		
				...
				if (_manager.GetEntityOfType<Script_SpaceShip> () != null) {
					if (_manager.GetEntityLocation<Script_SpaceShip> () == _gridPosition) {						
